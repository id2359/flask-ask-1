...
def setExpenditure(date,description,add_to_budget,amount,category):
	f = open("data.json","r")
	data = json.load(f)
	x = { "insertedAt" : date, "description" : description, "addToBudget" : str(add_to_budget), "amount" : int(amount) , "category" : category}
	...
	if add_to_budget == True:
		...
		data[0]["budgeting"]["expenditure"]["total"] += int(amount)
...
def setBudget(amount,category="all"):
	...
	f.close()
	if category == "all":
		data[0]["budgeting"]["budget"]["miscellaneous"] = int(amount) - data[0]["budgeting"]["budget"]["total"] + data[0]["budgeting"]["budget"]["miscellaneous"]
		data[0]["budgeting"]["budget"]["total"] = int(amount)
	else:
		data[0]["budgeting"]["budget"]["total"] = data[0]["budgeting"]["budget"]["total"] - data[0]["budgeting"]["budget"][category] + int(amount)
		data[0]["budgeting"]["budget"][category] =  int(amount)
	f1 = open("data.json","w")
	json.dump(data,f1)
	f1.close()
...
def RemainingBudget(category):
	f = open("data.json","r")
	data = json.load(f)
	if category == "all":
		budget = data[0]["budgeting"]["budget"]["total"]
		spent = data[0]["budgeting"]["expenditure"]["total"]
	else:
		budget = data[0]["budgeting"]["budget"][category]
		spent = data[0]["budgeting"]["expenditure"][category]
	f.close()
	return budget-spent
